...
async function main() {
  ...
  const step3 = await addStep({
    ...
    config: {
      publicCandidateBrief: 'Stai per chiamare Roberto Esposito, COO di Impresa Edile Esposito SpA (350 dipendenti, 25 cantieri attivi in tutta Italia). Ha compilato il form richiedendo una demo e ha menzionato "un problema urgente con la pipeline". Non sai altro. Il tuo obiettivo: capire il suo vero problema prima di parlare del prodotto.',
      aiPersona: {
        name: 'Roberto Esposito',
        role: 'COO',
        company: 'Impresa Edile Esposito SpA',
        personality: 'busy, direct, skeptical but open if asked right questions',
      },
      hiddenPersonaContext: {
        realPain: 'I project managers inviano aggiornamenti via WhatsApp. Il COO non ha visibilità real-time sulla pipeline. Hanno perso 2 contratti il mese scorso perché nessuno aveva seguito up su lead caldi.',
        budget: '30.000-50.000€ annui, decisione entro fine trimestre',
        decisionMaker: 'Roberto è il decisore finale. Coinvolge anche il CEO solo per importi >100k.',
        urgency: 'Alta: hanno una riunione del CdA tra 6 settimane dove devono presentare un piano di digitalizzazione.',
        objections: ['Ho già provato un altro software e non ha funzionato', 'Il mio team non vuole cambiare strumenti'],
      },
      hiddenScoringRubric: {
        discoveredPain: 'Did candidate uncover WhatsApp chaos and visibility problem?',
        discoveredBudget: 'Did candidate ask about budget range?',
        discoveredUrgency: 'Did candidate find the board meeting deadline?',
        discoveredDecisionMaker: 'Did candidate identify Roberto as decision maker?',
        avoidedEarlyPitch: 'Did candidate avoid pitching the product before understanding the problem?',
        questionQuality: 'Were questions open-ended and follow-up driven?',
      },
      ...
      notifications: [
        {
          ...
          hiddenCorrectAction: 'reply',
          hiddenRationale: 'High urgency + technical question = immediate reply, send docs, book technical call',
        },
        {
          id: 'notif-2',
          channel: 'slack',
          ...
          hiddenCorrectAction: 'reply',
          hiddenRationale: 'Manager check-in = immediate brief reply, build relationship',
        },
        {
          id: 'notif-3',
          channel: 'email',
          ...
          hiddenCorrectAction: 'reply',
          hiddenRationale: 'Warm inbound re-engaging = immediate reply to book call while interest is fresh',
        },
        {
          id: 'notif-4',
          channel: 'email',
          ...
          hiddenCorrectAction: 'ignore',
          hiddenRationale: 'Newsletter = ignore or schedule for later, not urgent',
        },
        {
          id: 'notif-5',
          channel: 'email',
          ...
          hiddenCorrectAction: 'schedule_later',
          hiddenRationale: 'Low-intent inbound with no urgency signal = send pricing info but not high priority now',
        },
      ],
      scoringRubric: {
        criticalActions: ['notif-1-reply', 'notif-2-reply', 'notif-3-reply'],
        mustIgnore: ['notif-4-ignore'],
        responseQuality: 'Are replies professional, brief, and action-oriented?',
      },
    },
  });

...
export default function CandidateDetailPage() {
  ...
  if (!data) return <div className="text-red-600">Could not load results.</div>;

  const { result, submissions, events } = data;

  return (
    <div className="space-y-6 max-w-4xl">
      <h1 className="text-2xl font-bold">Candidate Result</h1>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 flex gap-8">
        ...
        {result.skillScores && Object.keys(result.skillScores).length > 0 && (
          <div className="border-l border-gray-200 pl-8 flex-1">
            <div className="text-sm font-medium text-gray-700 mb-2">Skill Scores</div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-200 font-semibold">Step Results</div>
            ...
            <div key={sub.id} className="px-5 py-4">
              ...
              {sub.stepType === 'simulated_call' && sub.answer?.transcript?.length > 0 && (
                <details className="mt-3">
                  <summary className="text-xs text-blue-600 cursor-pointer hover:underline">Show transcript ({sub.answer.transcript.length} turns)</summary>
                  <div className="mt-2 space-y-2 max-h-64 overflow-auto bg-gray-50 rounded p-3">
                    {sub.answer.transcript.map((t: any, i: number) => (
                      <div key={i} className={`text-xs ${t.speaker === 'candidate' ? 'text-blue-800' : 'text-gray-700'}`}>
                        <span className="font-medium">{t.speaker === 'candidate' ? 'CANDIDATE' : 'BUYER'}: </span>{t.text}
                      </div>
                    ))}
                  </div>
                </details>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h3 className="font-semibold mb-3">Session Timeline</h3>
